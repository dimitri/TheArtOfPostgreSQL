            ...
            (let* ((cmdlist (list "git" "log"
                   ...
                   (git-log
                     (uiop:with-current-directory (path)
                       (format t "~{~a~^ ~}~%" cmdlist)
                       ;; git never re-encodes author/committer names (only
                       ;; the message body honors --encoding), so older repos
                       ;; can carry non-UTF-8 bytes in those fields. Decode
                       ;; leniently rather than aborting the whole import over
                       ;; one bad commit.
